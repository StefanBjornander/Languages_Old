...
(defconst eqc-version "Development"
  "The version number of EQC mode.")
...
(defvar eqc-gen-menu-items
  '(("Generators"
	 (("Sized generator" tempo-template-eqc-sized_gen)
	  ...
	  ('fun "weighted_default" "gen-weighted_default" ("Default ({weight,Value})" 'gen))
...
(defvar eqc-statem-menu-items
  '(("State machine specs"
	 (("Complete eqc_statem spec" tempo-template-eqc-eqc_statem)
	  ...
	  ('fun "zip"                "statem-zip"            ("List 1" "List 2"))
...
(defvar eqc-ver-menu-items
  '(("Version" eqc-version))
  "Description of the version of the Eqc mode.")
...
(defun expand-menu (items)
	...
	(while items
	  ...
	  (set (car items) (eqc-split-long-menus (symbol-value (car items)) 0))	  
	  (setq items (cdr items)))
  )

;; Splitting menus with more than 'eqc-max-menu-length' entries
...
(tempo-define-template
   ...
   '( "prop_" (p "Property name (prop_NAME): " prop) "() ->" > n
	  ...
	  (p "Module for commands [?MODULE]: " module t)
...
(defun eqc-version ()
  ...
  (interactive)
  (if (interactive-p)
      (message "EQC mode version %s, provided by Quviq AB"
	       eqc-version))
  eqc-version)



(defconst eqc-version "1.18")
